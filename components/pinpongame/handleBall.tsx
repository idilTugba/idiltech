...
export default function HandleBall({ sticksRef }: { sticksRef: SticksInfo }) {
  ...
  const [castleHeightElement, setCastleHeightElement] = useState<
    number | undefined
  >();
  ...
  const handleBall = () => {
    ...
    if (
      ...
      newBallInformation.y + 8 <
        sticksRef.leftStickTop + sticksRef.stickHeight &&
    ...
    if (
      ...
      newBallInformation.y + 8 <
        sticksRef.rightStickTop + sticksRef.stickHeight &&
    ...
    //Goal
    //Area of Castles
    const castleTopPoint = castleHeightElement
      ? (gameAreaSize.y - castleHeightElement) / 2
      : 0;
    const castleBottomPoint = castleHeightElement
      ? (gameAreaSize.y - castleHeightElement) / 2 + castleHeightElement
      : 0;
    ...
    if (
      ...
      if (newBallInformation.x + 8 >= gameAreaSize.x - 26)
        setScore((score) => ({ ...score, leftPlayer: score.leftPlayer + 1 }));
      if (
        newBallInformation.x <= 17 ||
        newBallInformation.x + 8 >= gameAreaSize.x - 26
      ) {
        resetBall();
      }
    }
  ...
  const resetBall = () => {
    ...
    newBallInformation.xSpeed = 3;
    newBallInformation.ySpeed = 3;
  ...
  return (
    <>
      ...
      <>
        ...
        ></div>
        <div className={styles.castle + " -right-2 bg-[#90ee90]"}></div>
      </>
      <p
